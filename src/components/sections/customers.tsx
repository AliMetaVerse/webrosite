      ...
      <div className="container-x">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {testimonials.map((t) => (
            <figure
              key={t.name}
              className="flex flex-col rounded-3xl border border-ink-100 bg-ink-50/60 p-7"
            >
              <div className="flex gap-1 text-grape-500" aria-hidden>
              ...
              <figcaption className="mt-6 flex items-center gap-3">
                <span
                  className={`inline-flex h-11 w-11 items-center justify-center rounded-full text-sm font-semibold ${t.tone}`}
                >
                  {t.initials}
                </span>
                <span>
